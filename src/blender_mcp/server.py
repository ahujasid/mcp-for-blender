...
def preview_animation(ctx: Context, name: str, frames: list[int], view: str = "three_quarter",
    ...
    name identifies one scene object; frames contains 1–8 integers. view and max_size
...
async def validate_deformation(ctx: Context, name: str, frames: list[int], reference_frame: int,
    ...
    frames contains 1–100 integers, reference_frame is a known good pose. Compares world
...
async def inspect_rig(ctx: Context, name: str, offset: int = 0, limit: int = 50,
    ...
    name is an exact scene armature name; offset and limit (1–200) page sorted bones.
...
async def sculpt_region(ctx: Context, name: str, center: list[float], radius: float,
    ...
    .sculpt_mask point attribute. max_vertices is a cap (1–500000), not a target density.
...
async def export_asset(ctx: Context, names: list[str], filepath: str, animation_mode: str = "SCENE",
    ...
    Include armatures and dependencies explicitly in names (1–200). Requires Object
...
def preview_camera(ctx: Context, camera_name: str, max_size: int = 768, output_path: str = "") -> list:
    ...
    max_size is 128–2048 pixels along the longest side. Uses Workbench shading and the
...
def preview_target(ctx: Context, name: str, view: str = "front", max_size: int = 1000,
    ...
    three_quarter in world axes (front looks from negative Y). max_size: 128–2048 pixels.
...
async def inspect_scene(ctx: Context, offset: int = 0, limit: int = 50,
    ...
    offset: nonnegative matched-object index; limit: 1–200; query: case-insensitive name
    ...
    selection, parenting, collections, modifiers, units and mode. Read-only, not geometry
    validation. Names are not persistent IDs; re-query after renaming or changing the scene.
    """
...
async def validate_mesh(ctx: Context, name: str, triangle_budget: int | None = None,
    ...
    name: exact mesh object name; target must be in Object Mode. triangle_budget:
    optional nonnegative integer; area_epsilon: finite nonnegative degeneracy threshold

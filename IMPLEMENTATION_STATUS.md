...
Development build 1.9.1+quality.1, add-on 1.6.1, quality extension protocol 1. The upstream socket protocol remains 5. Root and bundled add-ons are identical.
...
All eighteen quality tools are registered with the MCP server. No worked examples, extra workflow guide or new inline code comments were added. Tool descriptions explain prerequisites, effects, evidence limits and appropriate follow-up checks.

## Validation

- All 123 pytest checks pass: the original 113 regressions and new failure/discovery contracts.
- Blender 5.2.1 LTS background fixtures pass: pagination, evaluated subdivision, open boundaries, intentional motion jumps, checkpoint save/restore, and background viewport rejection.
...
For hands-on testing, use a separate Blender session and launch the server from this fork or wheel. The ordinary published uvx blender-mcp command uses the upstream package rather than this development build. The tested local client configuration points to this fork's server and the matching add-on in the dedicated test session. Installing this repository does not publish a new upstream PyPI package.

Developer checks from this repository:

```text
.venv/Scripts/python.exe -m pytest -q
blender --background --factory-startup --python-exit-code 1 --python tests/blender_quality_smoke.py
blender --background --factory-startup --python-exit-code 1 --python tests/blender_advanced_smoke.py
